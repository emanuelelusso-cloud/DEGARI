...
class Table:
    ...
    def __init__(self, from_input, max_attrs):
        ...
        # per evitare il puro brute force, con max_attrs piccolo rispetto al numero di attributi totali
        if n_of_cols/3 >= max_attrs:
            self.table = self.create_table_combination(max_attrs, n_of_cols)
        else:
            self.table = self.create_table(max_attrs, n_of_cols)
    ...
    def create_table_combination(self, max_attrs, n_of_cols):
        min_attrs = 1
        ...
        # crea solo le combinazioni
        for i in range(1, max_attrs + 1):
            for combo in combinations(range(n_of_cols), i):
                bin_row = [0]*n_of_cols
                for j in combo:
                    bin_row[j] = 1
    ...
    def create_table(self, max_attrs, n_of_cols):
        min_attrs = 1
