...
def elaboraGraduatoria(prop_list, not_prop_list=[]):
    ...
    #####################################################################################################
    # Da cambiare tutto, i file sono già salvati in resume, quindi si apre quello e si cicla su id.
    # Non serve chiedere che file json si vuole
    # Inoltre si può direttamente usare artwork_output che contiene già tutti i record in ram
    #####################################################################################################
    ...
    with open(file) as json_file:
        ...
        descriptions, instance_id = acquire_json_fild(keys)
